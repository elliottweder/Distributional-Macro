...
md"""
The firm solves the following maximisation problem:
...
The household solves the following maximisation problem:

$\max c_{t+1} + v(L_{t}) + \lambda_{t}(y_{t} - p_{t}L_{t} - f_{t}S_{t} - D_{t}) + \mu_{t}(y_{t+1} + p_{t+1}L_{t} + (f_{t+1} + d_{t+1})S_{t+1} + (1 + r_{t+1})D_{t} - c_{t+1})$
...
answer_2 = md"""
We do not believe the model interest rate is consistent with interest rates in the real world. This may be because:

* No monetary policy or inflation in the model
	There is sufficient evidence that these factors change decision making (especially, interest rates) in models and the real world
...
md"""
We choose to look at the rising share of educated workers. The goal is to create a new steady state with a new value of ``\phi`` and compare the trends. 
"""
...
* Interest rate falls:
	Due to K/Y rising (discussed below)
* Decreasing share of intangible capital
	According to the paper, a higher ϕ should reduce investable assets, which reduces the interest rate and favours physical investments over intangible ones
...
More savings can be invested into capital, so $\frac{K}{Y}$ rises.
