...
# ╔═╡ 41034dd3-3480-40fd-936e-991887389048
using Roots: Brent
...
model_statistics = (; ratio = model_raw.ph / model_raw.y)
...
md"""
First, we plot the average consumption response to a fall in house prices over time. The average is falling over time because the consumption response to a fall in house prices decreases with age (becomes more negative), and the average age is rising (from 1980 to 2040). 
"""
...
answer2 = md"""
No, we would not use the model. There are several aspects of housing markets and empirical findings that are missing. 
...
data_statistics = (; ratio = data_raw.HOUSES / data_raw.INCOME)
...
age_dist = innerjoin(age_weights, cres_df, on = :age)

# ╔═╡ ca56a38e-0f2d-4539-add8-6ffed40b9301
age_dist.response = age_dist.share.*age_dist.c_res

# ╔═╡ 115bdf6a-1bb3-432f-9c88-33d0aa0fedcc
age_data = groupby(age_dist, :year)

# ╔═╡ eb9aff48-8317-4da8-b5e9-646a6fcfe45c
responses = combine(age_data, :response => mean)
