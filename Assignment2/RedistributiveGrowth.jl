...
get_prices(0.5, .7, mod)

# ╔═╡ 591d72b0-4f1c-44b9-92f1-60e44f49f950
# ╠═╡ disabled = true
#=╠═╡
get_prices(0.6,0.5, mod)
  ╠═╡ =#
...
# ╔═╡ 65792b8a-64df-4405-801c-6f59a1374195
begin 
...
# ╔═╡ 8fe5a506-b70a-4b7d-bbf3-f9737c407cc9
...
# ╔═╡ edc190d7-a994-4259-b8f4-bf2cc977bc50
begin 
	trends_vars_ϕ = compute_trends_variables(K_ϕ, H_ϕ, mod_ϕ)
	DataFrame([trends_vars, trends_vars_η, trends_vars_ϕ])
end

# ╔═╡ b2f202bb-ab04-4f1f-825a-67a697fbad8c
...
# ╔═╡ 92000fe2-443b-40c8-8b60-e20e9f04308c
...
# ╔═╡ 62af514e-97f0-479a-8c0f-f1f54f11c82b
mod2 = RedistributiveGrowthModel2()

# ╔═╡ 3030da0b-6535-427a-8c59-3ff86f640373
mod_s = RedistributiveGrowthModel2(s=mod2.s+0.2)

# ╔═╡ b89ce0a9-e5ee-41df-81ed-29cb3bcd788b
...
begin 
	K_init_s = 0.4
	H_init_s = 1.
	model_equations_3(K_init_s, H_init_s, mod_s)
end

# ╔═╡ a41a0a4d-bdd0-4392-83b6-e9f0e6f5e57a
res_s = optimize(x -> objective_function2(x, mod_s), [log(K_init_s), log(H_init_s)])

# ╔═╡ e2751910-e419-40a5-9f4e-b7647d22af09
(K_s, H_s) = exp.(Optim.minimizer(res_s))

# ╔═╡ 72bf2fdc-8914-4a95-83a3-df96205752db
begin 
	trends_vars_s = compute_trends_variables(K_s, H_s, mod_s)
	DataFrame([trends_vars, trends_vars_s])
end

# ╔═╡ 259016ae-90cd-4f03-8a08-dd2b11ee093d
trends(trends_vars_s, trends_vars)

# ╔═╡ c9bde4ee-2315-4d69-9981-2c7e3de869f6
answer_4 = md"""
Increasing the exogenous savings, gives us the same trend for interest rate, mortgage, prices and stock prices as in the secular trends. However, capital increases while intangible assets decrease. Further, there is no change in wage inequality. When savings increase, the RHS of the steady state equation increases. As there is more savings, companies invest more in capital increasing K. As K increases, the interest rate falls, increasing the stock prices and the prices. 
"""
