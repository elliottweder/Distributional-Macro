...
# ╔═╡ a8307292-060e-47f7-ab5d-95e205ab79b7
# ╠═╡ disabled = true
#=╠═╡
β = 0.96
  ╠═╡ =#

# ╔═╡ 215475b9-ef05-4e0f-9241-ef6625bc0d86
# ╠═╡ disabled = true
#=╠═╡
σ = 1.0
  ╠═╡ =#

# ╔═╡ bd666729-fe37-4296-b084-8ff048dc2d1b
# ╠═╡ disabled = true
#=╠═╡
u = log
  ╠═╡ =#

# ╔═╡ e2118c6b-84ef-43d4-ad70-af1fa5a24777
#=╠═╡
Household(; σ = 1.0, β = 0.96,
                      u = σ == 1 ? log : x -> (x^(1 - σ) - 1) / (1 - σ))
  ╠═╡ =#

# ╔═╡ 9f5418a7-9e43-4c57-bbfa-3350047f7489
#=╠═╡
(; β, u)
  ╠═╡ =#
...
function statespace(;
	...
	n_k = length(k_vals) # 200
	n_z = length(z_chain.state_values) # 3
...
# ╔═╡ 2f714405-bad4-45fb-b60f-1322c94a14b6
# ╠═╡ disabled = true
#=╠═╡
statespace(;
			k_vals = range(1e-10, 20.0, length = 200),
...
# ╔═╡ e79b53ba-1dba-40e6-8114-5bfda2c8a3f9
# ╠═╡ disabled = true
#=╠═╡
k_vals = range(1e-10, 20.0, length = 200)
  ╠═╡ =#

# ╔═╡ e78e09f5-d784-48eb-88eb-1ed5fd7ad411
#=╠═╡
states = 
		[(; k, z_draw) for k ∈ k_vals, z_draw ∈ z_chain.state_values] |> vec
  ╠═╡ =#

# ╔═╡ 1bd74f0d-1a64-4b86-ade3-5866522da927
#=╠═╡
n_k = length(k_vals)
  ╠═╡ =#

# ╔═╡ b0a71b5d-1f8b-4e1d-8321-affd97f140d5
# ╠═╡ disabled = true
#=╠═╡
n_z = length(z_chain.state_values)
  ╠═╡ =#

# ╔═╡ 39199c23-f81f-42d3-952a-2fe51b4e1e23
#=╠═╡
states_indices = 
		[(; k_i, z_draw_i) for k_i ∈ 1:n_k, z_draw_i ∈ 1:n_z] |> vec
  ╠═╡ =#

# ╔═╡ fc8e85d6-5091-4290-8f24-077291d67057
#=╠═╡
policies = 
	    [(; k_next, z) for k_next ∈ k_vals, z ∈ z_chain.state_values] |> vec
  ╠═╡ =#

# ╔═╡ d8c49c1d-4798-4a20-bf37-30c02ec9d8e3
#=╠═╡
policies_indices = 
	    [(; k_next_i, z_i) for k_next_i ∈ 1:n_k, z_i ∈ 1:n_z] |> vec
  ╠═╡ =#

# ╔═╡ 785a936f-d728-4234-a2f4-1e832f00b4a5
#=╠═╡
length(states) # 200 * 3 = 600
  ╠═╡ =#

# ╔═╡ ce2eb192-cff5-4acc-8ada-8c28c85ccfa7
#=╠═╡
length(policies)
  ╠═╡ =#
...
# ╔═╡ 8da0b31a-d49d-41c1-a7d7-4f4dc6dd454d
#=╠═╡
        ...
        for (i_pol, pol) ∈ enumerate(policies_indices)
            for (i_state, state) ∈ enumerate(states_indices)
				if next_state.k_i == pol.k_next_i
					...
					# why not z_chain.p[state.z_i, next_state.z_draw_i]
                end
            end
        end
    end
...
# ╔═╡ 166ac3ad-078d-48f1-8068-0f5bac029bbe
#=╠═╡
setup_Q!(Q, states_indices, policies_indices, z_chain)
  ╠═╡ =#

# ╔═╡ 5d40a242-77c3-43da-8f97-2d5a5eac63e3
#=╠═╡
for (i_next_state, next_state) ∈ enumerate(states_indices)
        for (i_pol, pol) ∈ enumerate(policies_indices)
            for (i_state, state) ∈ enumerate(states_indices)
...
# ╔═╡ 86956825-d472-492f-a62c-c75a41343748
#=╠═╡
Q
  ╠═╡ =#
...
# ╔═╡ 1786b4cc-e2b8-492c-9b71-f72f6eda9acc
#=╠═╡
R = zeros(length(states), length(policies))
  ╠═╡ =#

# ╔═╡ 09df5dd8-db9e-4a9b-9ec6-568313a01767
#=╠═╡
R
  ╠═╡ =#
...
function consumption((; k), (; k_next, z), (; q, w, Δr))
	# why not consumption((; k, z_draw), (; k_next, z), (; q, w, Δr))
	if k_next < 0 && Δr > 0
		r = (1/q - 1) + (k_next < 0) * Δr
		q = 1/(1+r)
	end
...
# ╔═╡ 10c03fc0-c6f5-426f-883d-6daf73c3b7ea
#=╠═╡
setup_R(states, states_indices, policies, policies_indices, prices, u)
  ╠═╡ =#

# ╔═╡ 5680495e-e6d6-4a69-b8c2-96c970080eac
#=╠═╡
for (p_i, policy) ∈ enumerate(policies)
		z_i_pol = policies_indices[p_i].z_i
        for (s_i, state) ∈ enumerate(states)
