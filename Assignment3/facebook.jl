...
md"""
The first thing we notice is that the correlation between own price and friend price changes falls the further away the friends are (the distance measure). For instance, when friends less than 300 miles are excluded, the correlation disappears when compared to the 'all friends' measure. This is because the house prices within a county are likely to be correlated, so including 'all friends' incorporates (retains) some of this information. Moreover, one tends to have more connections to friends who live in the same county, so they have a higher impact on the experienced house price change. As close friends are exluded (``x00``) then similar place are exclused and this within-county correlation disappears, and so does the correlation shown in the graphs. 

Secondly, we can see that in 2011 house prices drop (following the GFC) and that in 2021 house prices rise (COVID shock - people need somewhere to put their money). 

Also, there do not seem to be many outliers in the data. This is intriguing because it shows that people (aggregated at a county level) do not have friends in only one place, rather they are distributed evenly. Therefore, (weighted) changes in own county or friend county house prices do not have outliers. 

Lastly, when moving further away from the country (``x00``) then people experience, on average, almost the same friend house price changes. For example, along the Y-axis the variation stays around 5% (when using ``300``). We think that this is because the composition (for the weighted average) is not likely to change with x (``x00``). 
"""
...
md"""
First, the regression specification has 'amount' in logs, but we are not sure that in the joined_df dataframe that this is actually the case. This is because we cannot find in the code where 'amount' is logged. Therefore, we interpret the regression coefficients for a level-log model. From the specification given to us (with the initial variables, etc.) we have the following results:

Loan amount:
* For a 1% increase in the growth rate of house price of all friends (``friends\_exp\_000``) there is a ``\$\beta = \$1018.37`` change in the mortgage amount (p = 0.000)
* The growth rate of own county house price is not statistically related to a change in the mortgage amount (p = 0.827)

We are puzzled by this result, as we cannot see why changes in own county house prices are not related to loan amount.

When we take the log of 'amount' (as specified by the regression above) the growth rate of own county house price becomes almost significant (p = 0.105) and is associated with a 0.190604% decrease in loan amount. This is interesting because it shows that the econometric model matters for which variables are (almost) significant. For a 1% increase in the growth rate of house price of all friends, the loan amount increases by around 5% (p = 0.000.)

Originations:
* For a 1% increase in the growth rate of house price of all friends (``friends\_exp\_000``) there is a ``\beta = 0.018`` increase in the mortgage count per capita (p = 0.000)
* For a 1% increase in the growth rate of own county house price there is a ``\beta = 0.004`` increase in the mortgage count per capita (p = 0.003)

We are not fully convinced by the econometric model. 

We think that the IV specification may suffer from reverse causality. In particular, your friend group (all friends) is likely to influence your friend group outside of 100 miles (as well as 300 miles). At the same time, as the IV mechanism is defined, the friend group outside of 100 miles also determines your entire friend group. Therefore, since causality runs both ways, we think that the first stage IV regression is biased. To boot, we think that using ``friends\_exp\_300`` would be more plausibly exogenous than ``friends\_exp\_100``, because the friend groups are less correlated over the larger distance. However, when we change the model to include ``friends\_exp\_300`` as the instrument, the results (coefficients) change significantly. On top of that, in the originations regression (with ``friends\_exp\_300``) the coefficients lose their significance. This is worrying because it tells us that the IV specification is not robust.  

Why could this be? We think that using ``friends\_exp\_300`` could alter the results significantly because the sample size is too small or there is not enough variation (geographic). For example, someone in LA could have a big group of friends only in Detroit, but nowhere else outside of 300 miles from LA. It is difficult to say which effect plays a role here without looking through the data carefully. 

As an alternative, we think that using the location of an individual's highest educational attainment (i.e. where they went to high school, university, etc.) could be a valid and strong instrument. This is because the school creates a network of friends, but does not seem likely to be correlated to house prices. There will be enough variation in the instrument because people with a college education have more expansive and less clustered friend networks (Bailey et al. (2018)). This is because people from around the country go to attend a single university. However, we think that implementing this instrument may not be feasible due to data limitations. School attendance is observed at the individual level, while the study and data used here are at a county level. Nevertheless, we think that it would be possible to aggregate individual friend groups into a county level measure. Facebook has data on high school and universities, which helps us create the network of friends. 
"""
...
# ╔═╡ 1ed97ba5-2712-431b-b736-f9b41c9ab60e
reg(joined_df, 
	@formula(log(amount) ~ Δ_log_hpi + (friends_exp_000 ~ friends_exp_100) + fe(year)),
	#Vcov.cluster(:state),
	weights = :population2010
)

# ╔═╡ 1d5026a4-396d-4789-afa5-6b4a9aeb083d
reg(joined_df, 
	@formula(amount ~ Δ_log_hpi + (friends_exp_000 ~ friends_exp_300) + fe(year)),
	#Vcov.cluster(:state),
	weights = :population2010
)

# ╔═╡ e42bf651-24ce-4bfd-8a0f-2db070051ffa
reg(joined_df, 
	@formula(count_per_pop ~ Δ_log_hpi + (friends_exp_000 ~ friends_exp_300) + fe(year)),
	#Vcov.cluster(:state),
	weights = :population2010
)
